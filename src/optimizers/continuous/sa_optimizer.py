...
# def sa(fitness, dim, max_x, min_x, max_iter=30, 
#                             initial_temp=100, cooling_rate=0.9, neighbor_scale=0.02, seed=None):
#     if seed is not None:
#         np.random.seed(seed)

#     cur_pos = np.random.uniform(min_x, max_x, dim)
...
#         neigh = cur_pos + np.random.normal(0, 1, dim) * T * neighbor_scale
#         neigh = np.clip(neigh, min_x, max_x)
#         neigh_fit = fitness(neigh)

#         delta = neigh_fit - cur_fit
